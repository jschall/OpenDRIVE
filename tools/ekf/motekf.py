...
encoder_bias_pnoise = Symbol('encoder_bias_pnoise')
...
z = toVec(theta_e_m, i_ab_m) # Observation vector
R = diag(theta_e_m_noise**2,i_noise**2,i_noise**2) # Covariance of observation vector

# States
omega_r_est, theta_e_est, i_d_est, i_q_est, T_l_est, encoder_bias = symbols('state[0:6]')
x = toVec(omega_r_est, theta_e_est, i_d_est, i_q_est, T_l_est, encoder_bias)
nStates = len(x)
...
Q = G*Q_u*G.T
Q += diag(0**2, 0**2, 0**2, 0**2, T_l_pnoise**2, encoder_bias_pnoise**2)
...
h[2,0] = theta_e_est-encoder_bias
...
def print_code():
    ...
    init_P = upperTriangularToVec(diag(100., math.pi**2, i_noise**2, i_noise**2, 0.1**2, math.pi**2))
